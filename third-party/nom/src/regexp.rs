...
mod tests {
	...
	fn re_capture() {
		...
		assert_eq!(
			...
			Done("pouet", vec!["nom 0.3.11", "nom", "0.3.11", "0", "3", "11"])
		);
		assert_eq!(rm("blah"), Error(error_code!(ErrorKind::RegexpCapture)));
		assert_eq!(
			rm("hello nom 0.3.11 world regex 0.1.41"),
			Done(" world regex 0.1.41", vec!["nom 0.3.11", "nom", "0.3.11", "0", "3", "11"])
		);
	}
	...
	fn re_captures() {
		...
		assert_eq!(
			rm("blah nom 0.3.11pouet"),
			Done("pouet", vec![vec!["nom 0.3.11", "nom", "0.3.11", "0", "3", "11"]])
		);
		assert_eq!(rm("blah"), Error(error_code!(ErrorKind::RegexpCapture)));
		assert_eq!(
	...
	fn re_bytes_matches() {
		...
		assert_eq!(
			...
			Done(&b"pouet"[..], vec![&b"2015-09-07"[..], &b"2015-09-09"[..]])
		);
	}
	...
	fn re_bytes_captures() {
		...
		assert_eq!(
			...
			Done(
				&b"pouet"[..],
				vec![vec![&b"nom 0.3.11"[..], &b"nom"[..], &b"0.3.11"[..], &b"0"[..], &b"3"[..], &b"11"[..]]]
			)
		);
		assert_eq!(rm(&b"blah"[..]), Error(error_code!(ErrorKind::RegexpCapture)));
		assert_eq!(
