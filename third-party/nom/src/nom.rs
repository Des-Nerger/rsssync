...
use traits::{AsChar, InputIter, InputLength};
	...
	T: Compare<&'static str>,
{
	match input.compare("\r\n") {
		...
		CompareResult::Ok => IResult::Done(input.slice(2..), input.slice(0..2)),
...
pub fn alpha<T>(input: T) -> IResult<T, T>
where
	T: Slice<Range<usize>> + Slice<RangeFrom<usize>> + Slice<RangeTo<usize>>,
	T: InputIter + InputLength,
{
...
pub fn float(input: &[u8]) -> IResult<&[u8], f32> {
	flat_map!(
		...
		recognize!(tuple!(
			opt!(alt!(tag!("+") | tag!("-"))),
			alt!(delimited!(digit, tag!("."), opt!(digit)) | delimited!(opt!(digit), tag!("."), digit)),
			opt!(complete!(tuple!(alt!(tag!("e") | tag!("E")), opt!(alt!(tag!("+") | tag!("-"))), digit)))
		)),
		parse_to!(f32)
	)
}
...
mod tests {
	...
	fn offset() {
		...
		match alpha(a) {
			...
			_ => {
				panic!("wrong return type in offset test for alpha")
			}
		}
		match digit(b) {
			Done(i, _) => {
			...
			_ => {
				panic!("wrong return type in offset test for digit")
			}
		}
		match alphanumeric(c) {
			...
			_ => {
				panic!("wrong return type in offset test for alphanumeric")
			}
		}
		match space(d) {
			Done(i, _) => {
			...
			_ => {
				panic!("wrong return type in offset test for space")
			}
		}
		match multispace(e) {
			Done(i, _) => {
			...
			_ => {
				panic!("wrong return type in offset test for multispace")
			}
		}
		match hex_digit(f) {
			Done(i, _) => {
			...
			_ => {
				panic!("wrong return type in offset test for hex_digit")
			}
		}
		match oct_digit(f) {
			Done(i, _) => {
			...
			_ => {
				panic!("wrong return type in offset test for oct_digit")
			}
		}
	}
	...
	fn configurable_endianness() {
		...
		assert_eq!(
			...
			Done(&b""[..], 1297142246100992000_u64)
		);
		assert_eq!(
			le_tst64(&[0x12, 0x00, 0x60, 0x00, 0x12, 0x00, 0x80, 0x00]),
			Done(&b""[..], 36028874334666770_u64)
		);
		...
		assert_eq!(
			...
			Done(&b""[..], 71881672479506432_i64)
		);
		assert_eq!(
			le_tsti64(&[0x00, 0xFF, 0x60, 0x00, 0x12, 0x00, 0x80, 0x00]),
			Done(&b""[..], 36028874334732032_i64)
		);
	}
