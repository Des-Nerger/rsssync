...
impl<O: Clone, E: Copy, M: Copy> ConsumerState<O, E, M> {
	pub fn map<P, F>(&self, f: F) -> ConsumerState<P, E, M>
	where
		F: FnOnce(O) -> P,
	{
		match *self {
	...
	pub fn flat_map<P, F>(&self, f: F) -> ConsumerState<P, E, M>
	where
...
pub trait Producer<'b, I, M: 'b> {
	...
	fn run<'a: 'b, O, E: 'b>(&'b mut self, consumer: &'a mut dyn Consumer<I, O, E, M>) -> Option<&O> {
...
impl<'x> MemProducer<'x> {
	pub fn new(buffer: &'x [u8], chunk_size: usize) -> MemProducer {
		MemProducer { buffer: buffer, chunk_size: chunk_size, length: buffer.len(), index: 0 }
	}
}
		...
		File::open(&Path::new(filename)).and_then(|mut f| {
			f.seek(SeekFrom::Start(0)).map(|_| {
				...
				FileProducer {
					...
					state: FileProducerState::Normal,
				}
			})
		})
	}
...
impl<'a, R, S: Clone, T, E: Clone, M: Clone, F: Fn(S) -> T, C: Consumer<R, S, E, M>>
...
impl<'a, R, S: Clone, T, E: Clone, M: Clone, F: Fn(S) -> T, C: Consumer<R, S, E, M>> Consumer<R, T, E, M>
	for MapConsumer<'a, C, R, S, T, E, M, F>
{
...
impl<'a, 'b, R, S: Clone, T: Clone, E: Clone, M: Clone, C1: Consumer<R, S, E, M>, C2: Consumer<S, T, E, M>>
...
mod tests {
	...
	impl<'a> Consumer<&'a [u8], &'a [u8], (), Move> for AbcdConsumer<'a> {
		fn handle(&mut self, input: Input<&'a [u8]>) -> &ConsumerState<&'a [u8], (), Move> {
			match input {
				...
				Input::Element(sl) => {
					match abcd(sl) {
						IResult::Error(_) => self.state = ConsumerState::Error(()),
						IResult::Incomplete(_) => self.state = ConsumerState::Continue(Move::Consume(0)),
						IResult::Done(i, o) => self.state = ConsumerState::Done(Move::Consume(sl.offset(i)), o),
					};
					&self.state
				}
				Input::Eof(Some(sl)) => {
	...
	fn mem2() {
		...
		let mut a =
			StateConsumer { state: ConsumerState::Continue(Move::Consume(0)), parsing_state: State::Initial };
	...
	fn seeking_consumer() {
		...
		let mut a =
			SeekingConsumer { state: ConsumerState::Continue(Move::Consume(0)), position: SeekState::Begin };
		for _ in 1..4 {
