...
mod tests {
	...
	fn escape_transform() {
		...
		assert_eq!(
			...
			Error(error_node_position!(
				...
				&b"AB\\"[..],
				error_position!(ErrorKind::EscapedTransform, &b"\\"[..])
			))
		);
		assert_eq!(
			esc(&b"AB\\A"[..]),
	...
	fn take_until_test() {
		...
		assert_eq!(x(&b"ab"[..]), Incomplete(Needed::Size(4)));
	}

	#[test]
	fn take_until_either() {
		named!(x, take_until_either!("!."));
		assert_eq!(x(&b"123!abc"[..]), Done(&b"!abc"[..], &b"123"[..]));
	}

	#[test]
	fn take_until_either_incomplete() {
		named!(x, take_until_either!("!."));
		assert_eq!(x(&b"123"[..]), Error(error_position!(ErrorKind::TakeUntilEither, &b"123"[..])));
	}
	...
	fn take_until_incomplete() {
		...
		assert_eq!(y(&b"nd"[..]), Incomplete(Needed::Size(3)));
		assert_eq!(y(&b"123"[..]), Error(error_position!(ErrorKind::TakeUntil, &b"123"[..])));
	}
