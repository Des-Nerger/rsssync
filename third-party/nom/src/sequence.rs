...
mod tests {
	...
	fn err() {
		...
		assert_eq!(
			res_a,
			Error(error_node_position!(ErrorKind::Custom(42), blah, error_position!(ErrorKind::Tag, blah)))
		);
		assert_eq!(
			res_b,
	...
	fn terminated() {
		...
		assert_eq!(
			...
			Error(error_position!(ErrorKind::Tag, &b"xxxxdef"[..]))
		);
	...
	fn delimited() {
		...
		assert_eq!(
			...
			Error(error_position!(ErrorKind::Tag, &b"xxxdefghi"[..]))
		);
		assert_eq!(
			delimited_abc_def_ghi(&b"abcxxxghi"[..]),
			Error(error_position!(ErrorKind::Tag, &b"xxxghi"[..]))
		);
