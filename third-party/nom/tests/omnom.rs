...
use nom::{Consumer, ConsumerState, IResult, Input, MemProducer, Move, Offset, Producer};
...
impl<'a> Consumer<&'a [u8], usize, (), Move> for TestConsumer {
	...
	fn handle(&mut self, input: Input<&'a [u8]>) -> &ConsumerState<usize, (), Move> {
		match self.state {
			State::Beginning => match input {
				Input::Empty | Input::Eof(None) => {
					self.state = State::Error;
					self.c_state = ConsumerState::Error(());
				}
				Input::Element(sl) | Input::Eof(Some(sl)) => match om_parser(sl) {
					IResult::Error(_) => {
						self.state = State::Error;
						self.c_state = ConsumerState::Error(());
					}
			...
			State::Middle => match input {
				Input::Empty | Input::Eof(None) => {
					self.state = State::Error;
					self.c_state = ConsumerState::Error(());
				}
				Input::Element(sl) | Input::Eof(Some(sl)) => match nomnom_parser(sl) {
					IResult::Error(_) => {
			...
			State::End => match input {
				Input::Empty | Input::Eof(None) => {
					self.state = State::Error;
					self.c_state = ConsumerState::Error(());
				}
				Input::Element(sl) | Input::Eof(Some(sl)) => match end_parser(sl) {
					IResult::Error(_) => {
						self.state = State::Error;
						self.c_state = ConsumerState::Error(());
					}
...
fn nom1() {
	...
	let mut c =
		TestConsumer { state: State::Beginning, counter: 0, c_state: ConsumerState::Continue(Move::Consume(0)) };
