...
use nom::IResult::*;
use nom::{be_f32, be_u16, be_u32, be_u64, ErrorKind, HexDisplay, IResult, Needed, Offset};
use nom::{Consumer, ConsumerState, FileProducer, FileProducerState, Input, Move, Producer};
				...
				match moov_header(sl) {
					Done(i, header) => {
						match header.tag {
							...
							_ => {
								println!("invalid header here: {:?}", header.tag);
								return ConsumerState::Error(());
							}
						};
						// TODO: check for overflow here
			...
			MP4State::Mvhd(sz) => match input {
				...
				Input::Element(sl) | Input::Eof(Some(sl)) => {
					...
					match self.c_state {
						...
						ConsumerState::Continue(Move::Seek(SeekFrom::Current(sz))) => {
							self.moov_bytes = self.moov_bytes - (sz as usize)
						}
						_ => (),
					};
					println!("remaining moov_bytes: {}", self.moov_bytes);
